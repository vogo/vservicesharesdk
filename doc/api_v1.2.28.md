...
4. sdk下载地址 [/images/sdk.zip](/images/sdk.zip)

!> 注：小程序内嵌此接口需要下载SDK文件放入到代码中 [微信小程序原生SDK](https://gitee.com/bubibi1/bosskg-demo/tree/master/%E5%B0%8F%E7%A8%8B%E5%BA%8Fsdk/SDK_MiniProgram)  [Uni打包微信小程序SDK](https://gitee.com/bubibi1/bosskg-demo/tree/master/%E5%B0%8F%E7%A8%8B%E5%BA%8Fsdk/SDK_Uni)
...
## 活体对接前配置
  ...
  一、为确保活体功能正常调用，小程序校验文件通过百度域名校验：<br/>
  （1）请在接口联调前3个工作日，向我方对接人提供自有小程序对应的域名校验文件（txt格式） <br/>
  （2）您的小程序中需增加百度的域名配置：https://h5-faceverify-upgrade.baidu.com<br/>
  （3）若贵方未提供域名校验文件或小程序中未增加百度域名配置，小程序调用活体认证接口时将返回错误，无法正常使用功能<br/>
  二、小程序开发方式说明，需明确是原生小程序开发、uni-app打包小程序开发，根据不同方式需配置文件不同：<br/>
  ...
  三、原生 APP WebView 加载 H5 摄像头调用（getUserMedia）配置说明：<br/>
...
| authChannel     | 是   | 字符串 | 2   | 认证通道    详情见5.26.2.1            |


### 5.26.2.1 authChannel 参数
| 认证通道ID   | 认证通道名称|
| ----------  | ------ | 
| 01   | 百度云|
| 02   | 阿里云|
